import React from 'react'

const CreditCard = () => {
  return (
    <div>
    <div className="relative border border-white m-auto h-52 md:h-52 w-80 rounded-xl bg-gradient-to-r from-gray-200 to-gray-50 backdrop-blur-sm shadow-2xl transition-transform sm:h-56 sm:w-96 sm:hover:scale-110">
    <div className="absolute top-2 w-full px-8 sm:top-2">
      <div className="flex justify-end">
        <img className="h-12 w-12 object-contain" src="data:image/png;base64," />
      </div>
      <div className="pt-1">
        <p className="tracking-more-wider font-normal text-zinc-700 text-2xl font-sans">4312  567  7890  7864</p>
      </div>
       <div className="inline-block my-3 w-12 h-8 bg-gradient-to-tl from-yellow-500 to-yellow-200 rounded-md shadow-inner overflow-hidden">
          <div className="relative w-full h-full grid grid-cols-2 gap-1">
            <div className="absolute border border-gray-900 rounded w-4 h-6 left-4 top-1"></div>
            <div className="border-b border-r border-gray-900 rounded-br"></div>
            <div className="border-b border-l border-gray-900 rounded-bl"></div>
            <div className=""></div>
            <div className=""></div>
            <div className="border-t border-r border-gray-900 rounded-tr"></div>
            <div className="border-t border-l border-gray-900 rounded-tl"></div>
          </div>
      </div>
      <div className="pt-2 pr-6 sm:pt-2">
        <div className="flex justify-between text-zinc-700">
          <div className="">
            <p className="text-xs font-light font-sans">Valid From</p>
            <p className="text-sm font-medium tracking-widest">03/22</p>
          </div>
          <div className="">
            <p className="text-xs font-light font-sans">Expiry</p>
            <p className="text-sm font-medium tracking-widest">03/25</p>
          </div>

          <div className="">
            <p className="text-xs font-light font-sans">CVV</p>
            <p className="tracking-more-wider text-sm font-bold">521</p>
          </div>
        </div>
      </div>
    </div>
  </div>
  </div>
  )
}


export default CreditCard